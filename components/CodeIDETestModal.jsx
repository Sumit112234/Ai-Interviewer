...
import { X, Code, Play, SkipForward, Sparkles, Zap, CheckCircle, Clock } from "lucide-react"
...
export default function CodeIDETestModal({ onIdeSubmit, data, setIdeData }) {
  ...
  const [totalTime, setTotalTime] = useState(data.time)
  const [timeRemaining, setTimeRemaining] = useState(data.time)
                ...
                <div className="flex-1 overflow-y-auto relative z-10">
                  ...
                  {timeRemaining < 60 && timeRemaining > 0 && (
                    <motion.div
                      ...
                      className="px-6 pb-4"
                    >
                      <div className="p-3 bg-red-900/30 border border-red-500/50 rounded-lg flex items-center space-x-2">
                        <Clock className="h-5 w-5 text-red-400 animate-pulse" />
                        <p className="text-sm text-red-200 font-medium">
                          Less than 1 minute remaining! Your code will be auto-submitted when time runs out.
                        </p>
                      </div>
                    </motion.div>
                  )}
...
//                   {/* Time Warning */}
//                   {false && timeRemaining < 60 && timeRemaining > 0 && (
//                     <motion.div
//                       initial={{ opacity: 0, y: -10 }}
//                       animate={{ opacity: 1, y: 0 }}
