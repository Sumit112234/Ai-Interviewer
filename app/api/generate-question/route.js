...
export async function GET(){
    ...
    let system = "answer in at least 10 words."
  ...
  return Response.json({ message: "Generate Question API is live." ,text}, { status: 200 })
}
  ...
  function formatProjects(p) {
    ...
    if (typeof p === "object") {
      const bits = [
        p.name ? `${p.name}` : "",
        p.description ? `— ${p.description}` : "",
        p.tech ? `Tech: ${Array.isArray(p.tech) ? proj.tech.join(", ") : String(p.tech)}` : "",
        p.impact ? `Impact: ${p.impact}` : "",
      ]
        .filter(Boolean)
        .join(" ")
...
// Check if candidate has programming skills
function hasProgrammingSkills(resumeData) {
  if (!resumeData) return false
  
  const programmingKeywords = [
    'javascript', 'python', 'java', 'c++', 'c#', 'ruby', 'php', 'swift', 'kotlin',
    'go', 'rust', 'typescript', 'react', 'angular', 'vue', 'node', 'django', 'flask',
    'programming', 'developer', 'engineer', 'coding', 'software'
  ]
  
  const skillsStr = (typeof resumeData.skills === 'string' 
    ? resumeData.skills 
    : Array.isArray(resumeData.skills) 
  ...
  const roleStr = (resumeData.roleAppliedFor || '').toLowerCase()
  
  return programmingKeywords.some(keyword => 
    skillsStr.includes(keyword) || roleStr.includes(keyword)
  )
}

// Calculate coding question probability (30% chance)
function shouldAskCodingQuestion(questionCount, codingQuestionsAsked, totalQuestions) {
  // Don't ask coding questions too early
  if (questionCount < 2) return false
  
  // Target: 30% coding questions
  const targetRatio = 0.3
  const currentRatio = totalQuestions > 0 ? codingQuestionsAsked / totalQuestions : 0
  
  // If we're below target, increase probability
  if (currentRatio < targetRatio) {
    return Math.random() < 0.4 // 40% chance to catch up
  }
  
  // If we're at or above target, reduce probability
  ...
  try {
    ...
    if (detectRepeatRequest(lastUser) && lastAI) {
      ...
      return Response.json({
        ...
        selectionFlags: [],
        memoryUpdates: [],
        scores: createDefaultTotals(),
        notes: "Repeated last question - no scoring applied",
        totals: safeTotals,
        emotion: "neutral",
        questionCountShouldIncrement: false,
        ide: false,
        codeType: null
      })
    }

    // Check if candidate can handle coding questions
    const canAskCoding = hasProgrammingSkills(resumeData)
    const requestCodingQuestion = canAskCoding && shouldAskCodingQuestion(
      questionCount, 
      codingQuestionsAsked, 
      questionCount
    )
...
- 30% of questions should be coding-related when candidate has programming skills`
    ...
    const prompt = promptTemplate
      ...
      .replace('{prevAI}', prevAI || '(none)')
      .replace('{lastAI}', lastAI || '(none)')
      .replace('{prevUser}', prevUser || '(none)')
      .replace('{lastUser}', lastUser || '(none)')
      .replace('{questionCount}', String(questionCount))
      .replace('{context}', context || '')
      .replace('{personality}', personality)
      .replace('{role}', resumeData?.roleAppliedFor || 'the role')
      .replace('{codingQuestionsAsked}', String(codingQuestionsAsked))
      .replace('{requestCodingQuestion}', String(requestCodingQuestion))
    ...
    // Validate coding question fields
    const ide = !!parsed.ide
    const validCodeTypes = ["write", "predict", null]
    const codeType = validCodeTypes.includes(parsed.codeType) ? parsed.codeType : null
    const codeSnippet = codeType === "predict" ? (parsed.codeSnippet || null) : null
    const expectedLanguage = codeType === "write" ? (parsed.expectedLanguage || null) : null
